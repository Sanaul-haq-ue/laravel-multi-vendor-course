...
abstract class AbstractSslCommerz implements SslCommerzInterface
{
    ...
    public function callToApi($data, $header = [], $setLocalhost = false)
    {
        ...
        if ($code == 200 & !($curlErrorNo)) {
            ...
            return json_encode([
                'failedreason' => 'FAILED TO CONNECT WITH SSLCOMMERZ API'
            ]);
    ...
    public function formatResponse($response, $type = 'checkout', $pattern = 'json')
    {
        ...
        if (!is_array($sslcz)) {
            return json_encode([
                'status' => 'fail',
                'data' => null,
                'message' => 'Invalid response from SSLCOMMERZ API'
            ]);
        }
        ...
        if ($type != 'checkout') {
            ...
            $gatewayUrl = $sslcz['GatewayPageURL'] ?? null;
            $logo = $sslcz['storeLogo'] ?? null;

            if ($gatewayUrl) {
                $status = (!empty($this->getApiUrl()) && $this->getApiUrl() === 'https://securepay.sslcommerz.com')
                    ? 'SUCCESS'
                    : 'success';
                ...
                $failedReason = $sslcz['failedreason'] ?? null;

                if ($failedReason && strpos($failedReason, 'Store Credential') === false) {
