                                                ...
                                                <table id="data-table" class="table text-nowrap mb-0 table-bordered">
                                                    <thead class="table-head">
                                                        <tr>
                                                            ...
                                                            <th class="bg-transparent border-bottom-0 wp-15">Tutor Details</th>
                                                    ...
                                                    <tbody class="table-body">
                                                        ...
                                                        <tr>
                                                            <td class="text-muted fs-14 fw-semibold">{{ $loop->iteration}}</td>

                                                            <td>
                                                                <div class="d-flex align-items-center">
                                                                    <span class="data-image avatar avatar-md rounded-circle" style="background-image: url({{asset('optional($approve->creator->email)->image')}})"></span>
                                                                    <div class="user-details ms-2">
                                                                        <h6 class="mb-0">{{$approve->creator->name}}</h6>
                                                                        <span class="text-muted fs-12">{{$approve->creator->email}}</span>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            ...
                                                            <td class="text-muted fs-13">
                                                                {{ $approve->approved_status == 0 ? 'Pending' : ($approve->approved_status == 1 ? 'Approved' : 'Unapproved') }}
                                                            </td>
                                                            <td>
                                                                <div class="d-flex align-items-stretch">
                                                                    
                                                                    <form action="{{route('courseApprovedSubmit',$approve->id)}}" method="POST">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <input type="hidden" name="approved_status" value='1'>
                                                                    <button type="submit" class="btn btn-sm btn-outline-primary border me-2" data-bs-toggle="tooltip" data-bs-original-title="Mark As Completed">
                                                                    ...
                                                                    <form action="{{route('courseRejectSubmit',$approve->id)}}" method="POST">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <input type="hidden" name="approved_status" value='2'>
                                                                    <button type="submit" class="btn btn-sm btn-outline-secondary border me-2" data-bs-toggle="tooltip" data-bs-original-title="Reject">
